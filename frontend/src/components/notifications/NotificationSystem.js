...
export const NotificationBell = () => {
  ...
  const getNotificationIcon = (type) => {
    ...
    if (
      type.includes("requirement_") ||
      type.includes("visa_") ||
      type.includes("permit_")
    ) {
      return (
        <svg
    ...
    if (type.startsWith("package_")) {
      if (
        ...
        return data?.packageId
          ? `/vacations/${data.packageId}`
          : "/tour-guide/my-submissions";
      }
              ...
              {loading ? (
                ...
                notifications.map((notification) => (
                  <Link
                    ...
                    className={`block px-5 py-4 hover:bg-neutral-50 transition-colors duration-150 ${
